              ...
              {itemList.map((item, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {item.imagem ? (
                      <div>
                        {item.imagemContentType ? (
                          <a onClick={openFile(item.imagemContentType, item.imagem)}>
                            <img src={`data:${item.imagemContentType};base64,${item.imagem}`} style={{ maxHeight: '30px' }} />
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {item.imagemContentType}, {byteSize(item.imagem)}
                        </span>
                      </div>
                    ) : null}
                  </td>
