...
public class CadastroUserResource {
    ...
    public ResponseEntity<CadastroUser> partialUpdateCadastroUser(
        ...
        Optional<CadastroUser> result = cadastroUserRepository
            ...
            .map(existingCadastroUser -> {
                ...
                if (cadastroUser.getDocumentoNumero() != null) {
                    existingCadastroUser.setDocumentoNumero(cadastroUser.getDocumentoNumero());
                }
    ...
    public List<CadastroUser> getAllCadastroUsers(@RequestParam(required = false) String filter) {
        if ("acao-is-null".equals(filter)) {
            log.debug("REST request to get all CadastroUsers where acao is null");
            return StreamSupport
                .stream(cadastroUserRepository.findAll().spliterator(), false)
                .filter(cadastroUser -> cadastroUser.getAcao() == null)
