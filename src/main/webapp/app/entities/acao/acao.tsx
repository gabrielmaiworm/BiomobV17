            ...
            <tbody>
              {acaoList.map((acao, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{acao.dataCriacao ? <TextFormat type="date" value={acao.dataCriacao} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  ...
                  <td>
                    {acao.foto ? (
                      <div>
                        {acao.fotoContentType ? <a onClick={openFile(acao.fotoContentType, acao.foto)}>Open &nbsp;</a> : null}
                        <span>
                          {acao.fotoContentType}, {byteSize(acao.foto)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  <td>{acao.observacoes}</td>
                  <td>{acao.doacaoItem ? <Link to={`doacao-item/${acao.doacaoItem.id}`}>{acao.doacaoItem.id}</Link> : ''}</td>
                  <td>{acao.cadastroUser ? <Link to={`cadastro-user/${acao.cadastroUser.id}`}>{acao.cadastroUser.id}</Link> : ''}</td>
                  <td>
                    {acao.solicitacaoItem ? <Link to={`solicitacao-item/${acao.solicitacaoItem.id}`}>{acao.solicitacaoItem.id}</Link> : ''}
                  </td>
