...
/**
 * REST controller for managing {@link org.com.biomob.domain.Item}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Item> result = itemRepository
            ...
            .map(existingItem -> {
                ...
                if (item.getImagem() != null) {
                    existingItem.setImagem(item.getImagem());
                }
                if (item.getImagemContentType() != null) {
                    existingItem.setImagemContentType(item.getImagemContentType());
                }
                if (item.getSaldo() != null) {
                    existingItem.setSaldo(item.getSaldo());
                }
                if (item.getCategoriaItem() != null) {
                    existingItem.setCategoriaItem(item.getCategoriaItem());
                }
                if (item.getUnidadeMedida() != null) {
                    existingItem.setUnidadeMedida(item.getUnidadeMedida());
                }
